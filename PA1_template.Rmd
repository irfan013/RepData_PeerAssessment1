...
activity monitoring devices such as a [Fitbit] (http://www.fitbit.com) , [Nike Fuelband] (http://www.nike.com/us/en_us/c/nikeplus-fuelband), or [Jawbone Up] (https://jawbone.com/up). These 
...
underutilized both because the raw data are hard to obtain and there is a lack of
...
We have to read the date first.

```{r}
echo = TRUE
setwd("D:/coursera/reproducible")
activityD <- NULL
activityD <- read.csv("activity.csv", header = T, sep = ",")
```

We have to convert the variables may be created by this code to NULL:
...
```{r}
echo = TRUE
su <- tapply(activityD$steps, activityD$date, sum, na.rm=T)
```
...
```{r}
echo = TRUE
mn_int <- tapply(activityD$steps, activityD$interval, mean, na.rm=T)
plot(mn_int ~ unique(activityD$interval), type="l", xlab = "5-min interval")
```
...
```{r}
echo = TRUE
table(is.na(activityD) == TRUE)
```
```{r}
summary(activityD)
```
Total number of missing values in the dataset equals 2304. 
...
```{r}
echo = TRUE
activity2 <- activityD  # creation of the dataset that will have no more NAs
for (i in 1:nrow(activityD)){
    if(is.na(activityD$steps[i])){
        activity2$steps[i]<- mn_int[[as.character(activityD[i, "interval"])]]
    }
}
```
...
summary(activity2)
```
That is to confirm whether there is anymore NAs in the steps variable or not.
